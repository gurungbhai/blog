...
function makeListHtml(props, result ) {
	...
	if( data.length > 0 ) {
		...
		//alert(data[i].status);	
			...
			if( !( data[i].status == 'PENDING' || !data[i].status == 'PENDING' ||  data[i].status == 'SCHEDULED' ) ) {
			...
			if( data[i].status == 'PENDING' || data[i].status == 'ACCEPTED' || data[i].status == 'STARTED' || data[i].status == 'ARRIVED' || data[i].status == 'PICKEDUP' || data[i].status == 'SCHEDULED' || data[i].status == 'PENDING' ) {
